...
`mytoken-api` also owns optional external model providers. Anthropic, DeepSeek, and generic OpenAI Chat/Responses credentials stay in API-only secret files and never cross into `mytoken-worker`:

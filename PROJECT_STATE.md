...
- Fifty-five automated tests pass, including Key IP/CIDR, request balance/concurrency policy, request logging, a two-request function-tool loop, deployment-script syntax/release checks, and the published CLI help contract.
...
- `0.1.0-preview.6` is published. Source is being prepared for `0.1.0-preview.7` with transactional runtime/environment rollback and explicit API/UI release verification; registry publication and the matching Git tag are still required before page-based updates can install it.

...
- Sixty automated tests pass, including Key IP/CIDR, request balance/concurrency policy, request logging, a two-request function-tool loop, Provider credential separation/migration, GitHub update selection, deployment-script syntax/release checks, and the CLI help contract.
...
- npm development publication is paused after `0.1.0-preview.7`. Source is prepared as GitHub-only `0.1.0-preview.8`; once its matching Git tag is pushed, page-based updates use GitHub directly.
